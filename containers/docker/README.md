...
## You can run PWSend in multiple containerized scenarios:
...
This scenario uses `docker-compose` and runs the app using two containers on a single host (PWSend and PostgreSQL); persistent storage for PostgreSQL is assured by using a volume on the host machine.
...
You can run PWSend in OpenShift in 2 ways:
...
- if you want the PWSend template to be available to ALL the projects (Other category in the catalog) in the cluster you need to create the template in the OpenShift namespace: `oc create -f template-with-buildconfig.yaml -n openshift`
